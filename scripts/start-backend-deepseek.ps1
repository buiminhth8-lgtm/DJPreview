# 启动后端（DeepSeek 线上 Provider profile）。
# 用法：
#   .\scripts\start-backend-deepseek.ps1
#   .\scripts\start-backend-deepseek.ps1 -Port 8010
#   .\scripts\start-backend-deepseek.ps1 -NoReload
# 不会打印任何 API key（含 DEEPSEEK_API_KEY）。
#
# 启动前请确认：
#   1. 已复制 example：Copy-Item .deepseek.env.example .deepseek.env
#   2. 已在 .deepseek.env 填入真实 DEEPSEEK_API_KEY（仅保留本地，不要提交）
...
$env:LLM_ENV_PROFILE = "deepseek"

$ExpectedEnv = Join-Path $ProjectRoot ".deepseek.env"
if (-not (Test-Path $ExpectedEnv)) {
  Write-Warning "Expected env file not found: .deepseek.env"
  Write-Warning "Copy example first: Copy-Item .deepseek.env.example .deepseek.env"
}

Write-Host "Starting backend with DeepSeek profile"
Write-Host "Expected env file: .deepseek.env"
Write-Host "This profile may call paid external API if configured"
